...
data_dir = 'C:/Users/steeleb/Dropbox/Lake Sunapee/monitoring/buoy data/data/all sensors/L0/winter hobo data/2022-2023WinterTemp/'
...
hobo_w22_1m <- read.csv(file.path(data_dir, 'U22-1m.csv'),
                        col.names = c('obs', 'datetime', 'TempC_1m', 'detach', 'attach', 'connect', 'stop', 'eof'),
                        skip =2) %>% 
...
hobo_w22_5m <- read.csv(file.path(data_dir, 'U22-5m.csv'),
                        ...
                        skip =2) %>% 
  select(-obs, -c(detach:eof))
hobo_w22_6m <- read.csv(file.path(data_dir, 'U22-6m.csv'),
...
# join together
hobo_w22 <- full_join(hobo_w22_1m, hobo_w22_2m) %>% 
  full_join(., hobo_w22_3m) %>% 
  full_join(., hobo_w22_4m) %>% 
  full_join(., hobo_w22_5m) %>% 
  full_join(., hobo_w22_7m) %>% 
  full_join(., hobo_w22_6m) %>% 
  full_join(., hobo_w22_8m) %>% 
  full_join(., hobo_w22_9m) %>% 
  full_join(., hobo_w22_10m)

rm(hobo_w22_1m, hobo_w22_2m, hobo_w22_3m, hobo_w22_4m, hobo_w22_5m, hobo_w22_6m, hobo_w22_7m, hobo_w22_8m, hobo_w22_9m, hobo_w22_10m)
